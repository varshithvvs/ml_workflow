...
def inputs(train_file, test_file, output_variable):
    ...
    # Load input files
    rd = Reader(sep=",")
    train_data = rd.clean(train_file)
    test_data = rd.clean(test_file)
    train_data = dd.from_pandas(train_data, npartitions=1)
    test_data = dd.from_pandas(test_data, npartitions=1)
